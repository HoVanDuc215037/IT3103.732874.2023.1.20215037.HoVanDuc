...
public class Cart {
	private static ObservableList<Media> items0rdered = FXCollections.observableArrayList();
	public static final int MAX_NUMBERS_ORDERED = 20;

	public ObservableList<Media> getItems0rdered() {
		return (ObservableList<Media>) items0rdered;
	}

	//addMedia() & removeMedia()
	public static void addMedia (Media m) throws PlayerException{ 
		if(items0rdered.size() < MAX_NUMBERS_ORDERED) { 
			//TODO add media into cart
			if (items0rdered.contains(m)) {
			...
			throw new PlayerException("ERROR: The number of " + "media has reached its limit");	
		}	
	}
